...
exports.index = function(req, res) {
    ...
    var context = {
        title: 'tlks.io : A curated and community driven list of technical talks',
        user: user
    };
...
exports.authTwitter = function(req, res) {
    ...
    consumer.getOAuthRequestToken(function(error, oauthToken, oauthTokenSecret, results) {
        if (error) {
            var context = {
                message: "Error getting OAuth request token : " + util.inspect(error)
            };
            res.status(500).render("500", context);
        } else {
            ...
            if (error) {
                ...
                consumer.get(
                    ...
                    req.session.oauthAccessTokenSecret, function(error, data, response) {
                        if (error) {
                            var context = {
                                message: "Error getting twitter screen name : " + util.inspect(err)
                            };
                            res.status(500).render("500", context);
                        } else {
