...
exports.index = function (req, res) {
    ...
    libtlks.talk.latest(config.mongodb, 5, function (err, talks) {
        if (err) {
            res.status(500).send("Error : " + util.inspect(err)).end();
        }
        context.latest = talks;
        libtlks.talk.popular(config.mongodb, 5, function (err, talks) {
            if (err) {
                res.status(500).send("Error : " + util.inspect(err)).end();
            }
                ...
                consumer.get(
                    ...
                    req.session.oauthAccessTokenSecret, function (error, data, response) {
                        if (error) {
                            ...
                            userModel.getByUsername(user.username, function (err, dbuser) {
