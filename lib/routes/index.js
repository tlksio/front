...
exports.authTwitter = function(req, res) {
    ...
    consumer.getOAuthRequestToken(function(error, oauthToken, oauthTokenSecret, results) {
        if (error) {
            context.message = "Error getting OAuth request token : " + util.inspect(error);
            res.status(500).render("500", context);
        } else {
            ...
            if (error) {
                ...
                consumer.get(
                    ...
                    req.session.oauthAccessTokenSecret, function(error, data, response) {
                        if (error) {
                            context.message = "Error getting twitter screen name : " + util.inspect(err);
                            res.status(500).render("500", context);
                        } else {
                            var parsedData = JSON.parse(data);
                            var user_url = parsedData.entities.url.urls[0].expanded_url || undefined;
                            var user = {
                                id: uuid.v1(),
